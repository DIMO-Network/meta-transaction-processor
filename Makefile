...
APPS = meta-transaction-processor
all: $(APPS)

install: $(APPS)
	@mkdir -p bin
	@cp $(PATHINSTBIN)/meta-transaction-processor ./bin/
...
docker: deps
	@docker build -f ./resources/docker/Dockerfile . -t dimozone/meta-transaction-processor:$(VER_CUT)
	@docker tag dimozone/meta-transaction-processor:$(VER_CUT) dimozone/meta-transaction-processor:latest

docker-cgo: deps
	@docker build -f ./resources/docker/Dockerfile.cgo . -t dimozone/meta-transaction-processor:$(VER_CUT)-cgo
	@docker tag dimozone/meta-transaction-processor:$(VER_CUT)-cgo dimozone/meta-transaction-processor:latest-cgo
...
test: $(APPS)
	...
	@$(PATHINSTBIN)/meta-transaction-processor test ./config/test/...
